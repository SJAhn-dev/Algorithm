        ...
        for(int fileIdx = 0; fileIdx < files.length; fileIdx++) {
            ...
            while(idx < fileLength) {
                ...
                if(isHead) {
                    ...
                    else { head.append(tempChar); }
                }
                else if(isNum) {
                    if(!Character.isDigit(tempChar)) {
                        isNum = false;
                        list.add(new Filename(head.toString(), number.toString(), file.substring(idx, fileLength), fileIdx));
                        break;
                    }
                    else { number.append(tempChar); }
                }
                
                if(idx == fileLength - 1) {
                    list.add(new Filename(head.toString(), number.toString(), null, fileIdx));
                    break;
                }
